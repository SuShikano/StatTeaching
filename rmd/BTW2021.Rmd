...
```{r}
var.slope.1 <- sum((gdp - mean(gdp))^2*(summary(lm.out.1)$sigma^2))/((sum((gdp - mean(gdp))^2))^2)
...
```{r}
var.slope.1.het <- sum((gdp - mean(gdp))^2*(lm.out.1$residuals^2))/((sum((gdp - mean(gdp))^2))^2)
...
```{r}
lm.out.3 <- lm(turnout ~ gdp + unemp.young )
summary(lm.out.3)
```
...
resid.lm.out <- lm(resid.sq ~ gdp + unemp.young)
summary(resid.lm.out)
```
...
heterosk.func <- lm(resid.sq.log ~ gdp + unemp.young)
...
The weights used in FGLS is strongly correlated with the GDP variable, which was used for the WLS estimation. 
